...
export default function ProjectDossierModal({ node, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-3xl bg-[#0c1622] border-2 border-black rounded-lg shadow-2xl p-5 md:p-8 flex flex-col gap-6 my-auto text-left max-h-[90vh] overflow-y-auto scanline-overlay"
      ...
      >
        ...
        <div className="flex items-start justify-between border-b-2 border-black pb-4">
          <div>
            ...
            <div className="text-[11px] font-mono text-gray-400 mt-1 flex flex-wrap gap-x-4">
          ...
          <button
            ...
            className="w-8 h-8 rounded border-2 border-black bg-[#1f3144] hover:bg-[#324d6b] text-white flex items-center justify-center font-silk text-sm"
          >
            ✕
          </button>
        </div>
        ...
        {node.problem && (
          <div className="space-y-1.5">
            ...
            <p className="text-xs md:text-sm font-mono text-gray-300 leading-relaxed bg-[#111f2e] p-3.5 rounded border border-[#1d334a]">
              {node.problem}
            </p>
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {node.tech.map((tool) => (
                <span
                  key={tool}
                  className="px-2.5 py-1 rounded bg-[#162536] border border-[#274059] text-gray-200 text-xs font-mono"
                >
                  {tool}
                </span>
              ))}
        ...
        <div className="pt-3 border-t border-[#1b2d40] flex justify-end">
          <button
            ...
            className="px-6 py-2 rounded border border-black bg-[#e8a838] hover:bg-[#ffd277] text-black font-silk text-xs font-bold uppercase tracking-wider transition-colors shadow"
          >
            CLOSE DOSSIER
          </button>
        </div>
